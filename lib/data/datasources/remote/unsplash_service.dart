...
class ImageService {
  ...
  late final String _basUrl;
  late final Dio _dio;

  ImageService._internal() {
    _apiKey = dotenv.env['KEY_UNS'] ?? '';
    if (_apiKey.isEmpty) {
      throw Exception('KEY_UNS no encontrada en .env');
    }

    _basUrl = dotenv.env['URL_UNS'] ?? '';
    if (_basUrl.isEmpty) {
      throw Exception('URL_UNS no encontrada en .env');
    }
      ...
      final List<Map<String, dynamic>> images =
        ...
        return {
          ...
          'url': {
            ...
            'small':
                photo['urls']?['small'] ?? 'https://default-image-url.com',
            'thumb':
